...
const checkLogin = () => {
  ...
  if (email === '' || !email.replace(/\s/g, '').length || !emailFilter.test(email)) {
    errorCount += 1; emailElement.style.border = '1px solid red';
  } else { emailElement.style.border = '1px solid green'; }
  if (password === '' || !password.replace(/\s/g, '').length) {
    errorCount += 1; passwordElement.style.border = '1px solid red';
  } else {
  ...
  if (errorCount > 0) {
    ...
    return;
  } else { successElement.innerHTML = ''; }
  loginButton.innerHTML = 'Please wait';
  loginButton.setAttribute('disabled', '');
  const details = { password: password, email: email };
  loginUser(details);
};
